...
export function makeCBRobot() {
  ...
  //  脚の作成
  const legGeometry
    = new THREE.BoxGeometry(legW, legLen, legD, seg, seg);
  ...
  //  腕の設定
  const armGeometry
    = new THREE.BoxGeometry(armW, armLen, armD, seg, 1);
  ...
  //  頭の設定
  const head = new THREE.Group;
  const headGeometry = new THREE.BoxGeometry(headW,headH,headD,seg,seg);
